...
export function CreateTab({ onCreateBatch }) {
  ...
  const handleSubmit = async () => {
    if (!newBatch.weight || !newBatch.source || !newBatch.jeweller) {
  ...
  return (
    <div className="max-w-xl mx-auto glass-panel p-8 rounded-[2.5rem] space-y-8 animate-slide-up relative overflow-hidden">
      ...
      <div className="space-y-5">
        <div className="grid grid-cols-2 gap-5">
          <div className="space-y-2">
            ...
            <input
              ...
              className="w-full bg-black border border-white/10 rounded-xl p-4 text-white focus:outline-none focus:border-gold-500 focus:ring-1 focus:ring-gold-500/50 transition placeholder:text-gray-500"
            />
          </div>
          <div className="space-y-2">
            ...
            <div className="relative">
              <select
                ...
                className="w-full bg-black border border-white/10 rounded-xl p-4 text-white focus:outline-none focus:border-gold-500 focus:ring-1 focus:ring-gold-500/50 transition appearance-none [&>option]:bg-black [&>option]:text-white"
              >
                {PURITY_OPTIONS.map(p => <option key={p} className="bg-black text-white">{p}</option>)}
              </select>
        ...
        <div className="space-y-2">
          <label className="text-[10px] uppercase text-gold-500 font-bold ml-1 tracking-widest">Jeweller / Owner *</label>
          <input
            type="text"
            placeholder="Your Business Name"
            value={newBatch.jeweller}
            onChange={(e) => setNewBatch({ ...newBatch, jeweller: e.target.value })}
            className="w-full bg-black border border-white/10 rounded-xl p-4 text-white focus:outline-none focus:border-gold-500 focus:ring-1 focus:ring-gold-500/50 transition placeholder:text-gray-500"
          />
        </div>
        <div className="space-y-2">
          ...
          <input
            ...
            readOnly
            className="w-full bg-zinc-900 border border-white/5 rounded-xl p-4 text-gray-400 cursor-not-allowed"
          />
        </div>
      </div>
      ...
      <button
        ...
        disabled={loading}
        className="w-full bg-yellow-500 text-black font-bold py-4 rounded-xl hover:bg-yellow-400 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 transition-all duration-300 shadow-lg"
      >
