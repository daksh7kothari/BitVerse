          ...
          <div className="glass-panel p-8 rounded-3xl">
            <div className="relative mb-6">
              <input
                ...
                className="w-full bg-rich-black-900 border border-white/10 rounded-2xl py-4 px-6 text-xl focus:outline-none focus:border-gold-500 focus:ring-1 focus:ring-gold-500/50 transition placeholder:text-gray-600"
              />
              <button
                onClick={handleSearch}
                disabled={loading}
                className="absolute right-2 top-2 bottom-2 bg-gradient-to-r from-gold-400 to-gold-600 text-black px-6 rounded-xl font-bold hover:shadow-[0_0_15px_rgba(234,179,8,0.4)] transition disabled:opacity-50"
              >
                {loading ? 'Searching...' : 'Verify'}
              </button>
            </div>
            {error && <p className="text-red-400 text-center text-sm font-medium animate-pulse">Record not found. This might be an uncertified batch.</p>}
